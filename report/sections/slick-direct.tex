...
\lstinputlisting[caption=Original case classes]{code/oo.scala}
These classes will be the values which we want to insert into and fetch from our database.

Second, to provide necessary metadata to query on our user and car objects, we define a \texttt{Table[T]}.
\lstinputlisting[caption=\texttt{Table[T]} definition]{code/table.scala}
...
\lstinputlisting[caption=\texttt{TableQuery[T]} definition]{code/tablequery.scala}
Queries are written with a similar syntax as Scala collections are operated.
The following examples show a handful of Slick queries and their corresponding translations to (simplified) SQL.\
\lstinputlisting[caption=Slick queries]{code/example-queries.scala}
...
\lstinputlisting[caption=Column extension methods API]{code/overridden-types.scala}
